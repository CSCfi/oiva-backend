...
import fi.minedu.oiva.backend.model.entity.json.TranslatedStringDeserializer;
import fi.minedu.oiva.backend.model.entity.json.TranslatedStringSerializer;
...
public class TranslatedString implements Serializable {
    ...
    protected static TranslatedString ofNimi(final List<Metadata> metadata) {
        return of(Seq.seq(metadata).map(m -> Tuple.tuple(m.kieli().toLowerCase(), m.nimi())));
    }

    protected static TranslatedString ofKuvaus(final List<Metadata> metadata) {
        return of(Seq.seq(metadata).map(m -> Tuple.tuple(m.kieli().toLowerCase(), null != m.kuvaus() ? m.kuvaus() : "")));
    }
