-- End existing lupa for Raseborg
WITH old_lupa AS (
    UPDATE lupa
        SET loppupvm = '2019-12-31', jarjestaja_ytunnus='0595859-2'
        WHERE diaarinumero = '249/532/2012' RETURNING *
),
     historia AS (
     ...
     new_lupa AS (
         INSERT INTO lupa (paatoskierros_id,
             ...
             SELECT paatoskierros_id,
                    ...
                    null,
                    '2019-12-11',
                    '{"esittelija": "Annika Bussman", "liitetiedosto": "Raseborg-saatio_2020.pdf", "esittelija_nimike": "Opetusneuvos"}'
             FROM old_lupa
             RETURNING *
     ),
     copy_maarays AS (
...
FROM lupa l
WHERE l.id = lupa_id
  AND l.diaarinumero = '18/532/2019'
